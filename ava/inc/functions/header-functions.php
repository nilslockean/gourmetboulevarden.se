...
?>
	<?php echo esc_attr( $position ); ?>">
		...
		<div class="sidebar--section mobile-menu">
			...
			<?php if ( $flyout === true && is_active_sidebar( 'flyout' ) || is_customize_preview() ) : ?>
...
if ( ! function_exists( 'ava_page_title' ) ) :
	...
	function ava_page_title() {
		...
		if ( is_archive() ) {
			...
			if ( is_category() ) {
				$page_title = sprintf( '%1$s %2$s', esc_html( 'Posted in', 'ava' ), esc_html( single_cat_title( '', false ) ) );
			} elseif ( is_tag() ) {
				$page_title = sprintf( '%1$s %2$s', esc_html( 'Tagged', 'ava' ), esc_html( single_tag_title( '', false ) ) );
			} elseif ( is_date() ) {
				if ( is_month() ) {
					$page_title = sprintf( '%1$s %2$s', esc_html( 'Archive:', 'ava' ), esc_html( get_the_time( 'F, Y' ) ) );
				} elseif ( is_year() ) {
					$page_title = sprintf( '%1$s %2$s', esc_html( 'Archive:', 'ava' ), esc_html( get_the_time( 'Y' ) ) );
				} elseif ( is_day() ) {
					$page_title = sprintf( '%1$s %2$s', esc_html( 'Archive:', 'ava' ), esc_html( get_the_time( get_option( 'date_format' ) ) ) );
				} else {
					$page_title = sprintf( '%1$s', esc_html( 'Archive:', 'ava' ) );
				}
			} elseif ( is_author() ) {
				...
				$page_title = sprintf( '%1$s %2$s', esc_html( 'Archive:', 'ava' ), esc_html( single_term_title( '', false ) ) );
			} else {
				$page_title = single_term_title( '', false ); }
		} elseif ( is_search() ) {
			if ( have_posts() ) :
				$page_title = sprintf( '%1$s "%2$s"', esc_html( 'Results for', 'ava' ), esc_html( get_search_query() ) );
					else :
						$page_title = sprintf( '%1$s', esc_html( 'Nothing Found', 'ava' ) );
					endif;

		} elseif ( is_cart() ) {
			$page_title = sprintf( '%1$s', esc_html( 'Cart', 'ava' ) );
		}
