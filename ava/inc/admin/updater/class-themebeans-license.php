	...
	class ThemeBeans_License {
		...
		public function license_control( $wp_customize ) {
			...
			$wp_customize->add_section(
				'license_key', array(
					'title' => esc_html__( 'License Key', 'ava' ),
				)
			);
			...
			$description = sprintf( esc_html__( 'Enter a license key to enable remote updates and access theme support for %s.', 'ava' ), $this->item_name );

			$url = 'https://kb.themebeans.com/article/14-license-activation-guide';

			$tooltip = esc_html__( 'Need help?', 'ava' );

			if ( $this->status() ) {

				if ( 'valid' === $this->status() ) {

					$description = esc_html__( 'Awesome! Your site is connected to ThemeBeans and ready for remote updates, and support if you need it.', 'ava' );

				} elseif ( 'expired' === $this->status() ) {

					/* translators: 1. theme name, 2. expiration date */
					$description = sprintf( esc_html__( 'Your license for %1$s expired on %2$s and is no longer connected to ThemeBeans. Please renew your license to activate remote updates and support.', 'ava' ), $this->item_name, $this->expiration() );

					// Set the tooltip URL to use the renewal link instead.
					$url = $this->get_renewal_link();

					$tooltip = esc_html__( 'Renew License', 'ava' );
			...
			$wp_customize->add_control(
				new ThemeBeans_License_Control(
					$wp_customize, 'themebeans_license[key]', array(
						...
						/* translators: theme name */
						'label'       => sprintf( esc_html__( '%s License', 'ava' ), $this->item_name ),
		...
		public function add_schedules( $schedules = array() ) {
			...
			$schedules['weekly'] = array(
				'interval' => 604800,
				'display'  => esc_html__( 'Once Weekly', 'ava' ),
			);
		...
		public function activate_license_notice() {
			...
			echo '<div class="notice notice-warning"><p>' . sprintf( esc_html__( '%1$sActivate your license key%2$s for %3$s to enable remote updates and theme support. %4$s', 'ava' ), '<a href="' . esc_url( admin_url( '/customize.php?autofocus[section]=license_key' ) ) . '">', '</a>', esc_html( $this->item_name ), '<a style="text-decoration: none; float: right;" href="' . esc_url( $url ) . '" target="_blank"><span class="dashicons dashicons-editor-help"></span></a>' ) . '</p></div>';
		}
		...
		public function expired_notice() {
			...
			echo '<div class="error"><p>' . sprintf( esc_html__( 'Your license for %1$s expired on %2$s. %3$sRenew your license or club membership%4$s to activate remote updates and support.', 'ava' ), esc_html( $this->item_name ), esc_html( $expired_on ), '<a href="' . esc_url( $this->get_renewal_link() ) . '" target="_blank">', '</a>' ) . '</p></div>';
		}
		...
		public function options_page() {
			...
			add_theme_page( esc_html__( 'Theme License', 'ava' ), esc_html__( 'Theme License', 'ava' ) . $notifcation, 'manage_options', 'themebeans-license-key', '__return_null' );
		}
		...
		public function activate_license() {
			...
			if ( ! isset( $response->license ) ) {
				$message = esc_html__( 'An error occurred, please try again.', 'ava' );
			} else {
					...
					switch ( $response->error ) {
						...
						case 'expired':
							$message = sprintf(
								...
								esc_html__( 'Your license expired on %1$s. %2$sClick here to renew &rarr;%3$s', 'ava' ),
						...
						case 'revoked':
							$message = esc_html__( 'Your license key has been disabled.', 'ava' );
							break;

						case 'missing':
							$message = esc_html__( 'Invalid license.', 'ava' );
							break;

						case 'invalid':
						case 'site_inactive':
							$message = esc_html__( 'Your license is not active for this URL.', 'ava' );
							break;

						case 'item_name_mismatch':
							$message = esc_html__( 'This appears to be an invalid license key.', 'ava' );
							break;

						case 'no_activations_left':
							$message = esc_html__( 'Your license key has reached its activation limit.', 'ava' );
							break;
			...
			if ( ! isset( $response->license ) ) {
				...
				if ( false === $response->success ) {
					...
					switch ( $response->error ) {
						...
						case 'expired':
							$message = sprintf(
								...
								esc_html__( 'Your license key expired on %s.', 'ava' ),
								date_i18n( get_option( 'date_format' ), strtotime( $response->expires, current_time( 'timestamp' ) ) )
							);
							break;
						...
						case 'missing':
							$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'ava' );
							break;
					...
					if ( 'valid' === $response->license ) {
						$message = esc_html__( 'Your theme is activated! Remote updates and theme support are enabled.', 'ava' );
						$success = true;
					}
				}
			}
