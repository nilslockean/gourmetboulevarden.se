...
if ( ! function_exists( 'ava_body_data' ) ) :
	/**
	...
	function ava_body_data() {
	    ...
	    /*
	     * Override variables based on the URL parameter.
	     */
	    ...
	    if ( isset( $_GET['single_product'] ) ) {
	        $single_product = $_GET['single_product'];
	    }
	    if ( isset( $_GET['shop_columns_size'] ) ) {
	        $shop_columns_size = $_GET['shop_columns_size'];
	    }
	    if ( isset( $_GET['shop_product_hover'] ) ) {
	        $shop_product_hover = $_GET['shop_product_hover'];
	    }
...
function ava_customizer_add_menu( $tooltip_position, $menu ) {
    ...
    printf( '<span class="customizer-add-menu"><button class="index-event-button customizer-event-button" data-customizer-event="index-add-menu" data-balloon-pos="%1s" data-balloon="' . esc_attr__( 'Add %2$s Menu', 'ava' ) . '"></button></span>',
	...
	function ava_entry_meta() {
	    ...
	    if ( 'post' == get_post_type() ) {
	    	...
	    	if ( $date_option || is_customize_preview() ) :
	        	printf( _x( '<span class="days-ago%s">%s ago </span>', '%s = human-readable time difference', 'ava' ), esc_attr( $byline_visibility ), esc_html( human_time_diff( get_the_time( 'U' ) ),  esc_html( current_time( 'timestamp' ) ) ) );
	        endif;

	        if ( $byline_option || is_customize_preview() ) :
		        printf( '<span class="byline' . esc_attr( $byline_visibility ). '"><span class="author vcard"><span class="screen-reader-text">%1$s</span> %2$s <a class="url fn n" href="%3$s">%4$s </a></span></span> ',
...
function ava_the_custom_logo( $light_option ) {
    ...
    if ( has_custom_logo( $light_option ) ) {
        ...
        if ( 'light' == $scheme && $light_logo ) {
            ...
            printf( '<div itemscope itemtype="http://schema.org/Organization"><a class="custom-logo-link" href="%1$s" rel="home" itemprop="url"><img class="custom-logo" src="%2$s" alt="%3$s" itemprop="logo"></a></div>',
                esc_url( home_url('/') ),
