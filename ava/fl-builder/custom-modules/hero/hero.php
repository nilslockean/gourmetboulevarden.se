...
class Ava_Hero extends FLBuilderModule {
   ...
   /**
    ...
    public function enqueue_scripts() {
        ...
        if( $this->settings && $this->settings->parallax == 'yes' && ! empty ( $this->settings->bg_image ) ) {
            $this->add_js( 'parallax', get_theme_file_uri( '/fl-builder/assets/js/parallax.js' ), array( 'jquery' ), '1.5.9', true );
        }
    ...
    public function render_background( $layout ) {
        ...
        // Background image
        if( $this->settings->bg_layout == 'image' && ! empty( $this->settings->bg_image ) ) {

            if ( $this->settings->parallax == 'no' ) {
                echo '<div class="bb--hero__image" style="background-image: url(' . esc_url( $this->settings->bg_image_src ) . ');"></div>';
            }
        }

        // Background video
        if( $this->settings->bg_layout == 'video' ) {
 ...
 */
FLBuilder::register_module('Ava_Hero', array(
